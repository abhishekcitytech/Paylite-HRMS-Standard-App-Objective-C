...
@interface DashboardMH ()

@end

@implementation DashboardMH
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    ...
    else if ([strIdentifier isEqualToString:@"5"])
    {
        ...
        else
        {
            ...
            //NSLog(@"flag : %@",[dictemp valueForKey:@"InOutFlag"]);
        }
    }
    else  if ([strIdentifier isEqualToString:@"786"])
    {
        ...
        if (code==0)
        {
            ...
            /*int code111=[strAccPlatform intValue];
            if (code111==0) {
                //account inactive
    ...
    if (indexPath.section==0)
    {
        ...
        else
        {
            ...
            if ([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPhone){
                if (screenSize.height==568.0f){
                    //5S
                    btnAttendencePunch.frame=CGRectMake(CGRectGetMidX(cell.frame)-80,CGRectGetMidY(cell.frame),180,60);
                }
                else if(screenSize.height == 667.0f){
                    //6
                    btnAttendencePunch.frame=CGRectMake(CGRectGetMidX(cell.frame)-80,CGRectGetMidY(cell.frame),200,60);
                }
                else if(screenSize.height == 736.0f){
                    //6Plus
                    btnAttendencePunch.frame=CGRectMake(CGRectGetMidX(cell.frame)-80,CGRectGetMidY(cell.frame),200,60);
                    ...
                    //XSMAX XR
                    btnAttendencePunch.frame=CGRectMake(CGRectGetMidX(cell.frame)-60,CGRectGetMidY(cell.frame),200,60);
                }
                else{
                    btnAttendencePunch.frame=CGRectMake(CGRectGetMidX(cell.frame)-80,CGRectGetMidY(cell.frame),180,60);
                }
