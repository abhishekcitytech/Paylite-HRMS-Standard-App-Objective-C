...
- (void)viewDidLoad
{
    ...
    app5=(AppDelegate *)[[UIApplication sharedApplication] delegate];
    ...
    _tabvLeave.backgroundView=nil;
    _tabvLeave.backgroundColor=[UIColor whiteColor];
    _tabvLeave.separatorColor=[UIColor clearColor];
    _tabvLeave.showsVerticalScrollIndicator=NO;
...
-(void)downloadMyLeave
{
    ...
    NSString *soapMessage = [NSString stringWithFormat:
                             ...
                             "<SOAP-ENV:Body> \n"
                             "<GetMyLeave xmlns=\"http://tempuri.org/\"><sCompanyID>%@</sCompanyID>,<sEmployeeID>%@</sEmployeeID>"
                             "</GetMyLeave> \n"
                             "</SOAP-ENV:Body> \n"
    ...
    [theRequest addValue: @"http://tempuri.org/ISelfService/GetMyLeave" forHTTPHeaderField:@"SOAPAction"];
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    {
        if ([elementName isEqualToString:@"GetMyLeaveResult"])
        {
            NSLog(@"rootis called..");
            arrMLeaveDetails=[[NSMutableArray alloc]init];
        }
        if ([elementName isEqualToString:@"mytable"]) //mytable  Leave
        {
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    if ([strIdentifier isEqualToString:@"1"])
    {
        ...
        if ([elementName isEqualToString:@"Department"])
        {
            objldt.Department=currentElementValue;
            currentElementValue=nil;
        }
        if ([elementName isEqualToString:@"LeaveTypeID"])
        {
        ...
        if ([elementName isEqualToString:@"Gender"])
        {
            objldt.Gender=currentElementValue;
            currentElementValue=nil;
        }
        if ([elementName isEqualToString:@"ReportedToStatus"])
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cellLable1=[[UILabel alloc] initWithFrame:CGRectMake(14,10, _tabvLeave.frame.size.width-10, 17)];
    cellLable2=[[UILabel alloc] initWithFrame:CGRectMake(14,CGRectGetMaxY(cellLable1.frame)+5,_tabvLeave.frame.size.width-10, 17)];
    cellLable3=[[UILabel alloc] initWithFrame:CGRectMake(14,CGRectGetMaxY(cellLable2.frame)+5, _tabvLeave.frame.size.width-10, 17)];
    ...
    cellLable1.text=obj.LeaveType;
    cellLable2.text=[NSString stringWithFormat:@"%@  -  %@",obj.LeaveFromDate,obj.LeaveToDate];
    cellLable3.text=[NSString stringWithFormat:@"%@ Days",obj.NoOfDays];
    
    if ([obj.Leaveimages isEqualToString:@""]) {
    }
    else{
        /*UIButton *btnAttch=[UIButton buttonWithType:UIButtonTypeCustom];
        btnAttch.frame=CGRectMake(CGRectGetWidth(_tabvLeave.frame)-64,CGRectGetMinY(cellLable2.frame),32,32);
    ...
    viewBottomCell.frame=CGRectMake(14,80,_tabvLeave.frame.size.width-28,60);
    ...
    UILabel *lblSeparator=[[UILabel alloc] initWithFrame:CGRectMake(15,149.7,_tabvLeave.frame.size.width-15,0.3)];
...
#pragma  mark - Apply Leave Method
- (IBAction)pressApplyLeave:(id)sender
{
    ...
    if ([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPhone)
    {
        if (screenSize.height==568.0f){
            //5S
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeave5S" bundle:nil];
        }
        else if(screenSize.height == 667.0f){
            //6
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeave6" bundle:nil];
        }
        else if(screenSize.height == 736.0f){
            //6Plus
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeave6Plus" bundle:nil];
        }
        else if(screenSize.height == 812.0f){
            //X
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeaveX" bundle:nil];
        }
        else if(screenSize.height == 896.0f){
            //XSMAX XR
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeaveXSMAX" bundle:nil];
        }
        else{
            objDemo = [[ApplyLeave alloc] initWithNibName:@"ApplyLeave" bundle:nil];
        }
    }
    else
    {
